...
def remove_digit(values, box, digit):
    ...
    """
    new_values_string = values[box].replace(digit,'')
    return assign_value(values,box,digit)
        

def isTwin(pair,values):
...
def find_twins(unit, values):
    ...
    return [(pair[0],pair[1]) for pair in permutations(unit) if isTwin(pair,values)]
...
def eliminate_twins(unit, twins, values):
    ...
    for box in boxes_not_in_twin:
         remove_digit(values, box, digit1)
